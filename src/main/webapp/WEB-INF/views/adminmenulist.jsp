        ...
        <script>
              ...
              m.controller("menuInfo",function($scope , $http){
            	...
            	$scope.menuData = function()
            	{
            		...
            		$http.post("list").then(function(response){
            	...
            	{
            		$http.post("delete",rec).then(function(response){
            			if(response.data==1)
        		...
        		$scope.saveUpdateInfo=function(rec){
        			$http.post("update",rec).then(function(resp)
        			 		{
        	 					if(resp.data==1)
        	 					{
 ...
 <br><br>
  <center>
		<table>
				...
				<th>Menu Id</th>
				<th>Menu Name</th>
				<th>Menu Url</th><th></th><th></th>
				</thead>
										
					<tbody ng-repeat="x in menu">
					<tr>
					<td>{{x.id}}</td>
					<td>{{x.name}}</td>
					<td>{{x.url}}</td>
