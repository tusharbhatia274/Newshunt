        ...
        <script>
              ...
              m.controller("menuInfo",function($scope , $http){
            	...
            	$scope.menuData = function()
            	{
            		...
            		$http.post("clist").then(function(response){
            	...
            	{
            		$http.post("del",rec).then(function(response){
            			if(response.data==1)
        		...
        		$scope.saveUpdateInfo=function(rec){
        			$http.post("up",rec).then(function(resp)
        			 		{
        	 					if(resp.data==1)
        	 					{
 ...
 <br><br>
  <center>
		<table>
				...
				<th>Channel Id</th>
				<th>Channel Title</th>
				<th>Channel Url</th><th></th><th></th>
				</thead>
										
					<tbody ng-repeat="x in channel">
					<tr>
					<td>{{x.id}}</td>
					<td>{{x.title}}</td>
					<td>{{x.url}}</td>
